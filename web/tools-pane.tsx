...
export function ToolsPane({ tools }: { tools: ToolSummary[] }) {
  ...
  return (
    <section className="tools-pane">
      <header className="tools-pane-heading">
        <div>
          <span className="pane-kicker">Model tool contracts</span>
          <h2>Tools enabled in this chat</h2>
        </div>
        <span className="tool-count">{tools.length}</span>
      </header>
      <p className="tools-pane-note">The model sees each name, description, and argument schema. Agent Mom runs the chosen tool.</p>
      <div className="tools-pane-body">
        ...
        {selected && (
          <article className="tool-contract">
            <header>
              <div><span className="pane-kicker">Tool</span><h3>{selected.name}</h3></div>
            ...
            <section>
              <span className="pane-kicker">Argument schema sent to the model</span>
